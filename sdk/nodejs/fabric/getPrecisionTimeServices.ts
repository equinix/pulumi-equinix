 ...
 *
 * const all = equinix.fabric.getPrecisionTimeServices({
 *     pagination: {
 *         limit: 2,
 *         offset: 1,
 *     },
 ...
 *     }],
 * });
 * export const eptServiceId = all.then(all => all.datas?.[0]?.id);
 * export const eptServiceName = all.then(all => all.datas?.[0]?.name);
 * export const eptServiceState = all.then(all => all.datas?.[0]?.state);
 * export const eptServiceType = all.then(all => all.datas?.[0]?.type);
 * export const eptServiceIpv4 = all.then(all => all.datas?.[0]?.ipv4);
 * ```
 */
...
export interface GetPrecisionTimeServicesOutputArgs {
     ...
     * List of filters to apply to the stream attachment get request. Maximum of 8. All will be AND'd together with 1 of the 8 being a possible OR group of 3
     */
    filters?: pulumi.Input<pulumi.Input<inputs.fabric.GetPrecisionTimeServicesFilterArgs>[] | undefined>;
    /**
     * Pagination details for the returned route aggregations list
     */
    pagination?: pulumi.Input<inputs.fabric.GetPrecisionTimeServicesPaginationArgs | undefined>;
    /**
     * Filters for the Data Source Search Request
     */
    sorts?: pulumi.Input<pulumi.Input<inputs.fabric.GetPrecisionTimeServicesSortArgs>[] | undefined>;
}
