# coding=utf-8
# *** WARNING: this file was generated by pulumi-language-python. ***
# *** Do not edit by hand unless you're certain you know what you are doing! ***

import builtins as _builtins
import warnings
import sys
import pulumi
import pulumi.runtime
from typing import Any, Mapping, Optional, Sequence, Union, overload
if sys.version_info >= (3, 11):
    from typing import NotRequired, TypedDict, TypeAlias
else:
    from typing_extensions import NotRequired, TypedDict, TypeAlias
from .. import _utilities
from . import outputs
from ._enums import *

__all__ = [
    'DeviceBehavior',
    'DeviceIpAddress',
    'DeviceNetwork',
    'DevicePort',
    'DeviceReinstall',
    'GatewayTimeouts',
    'InterconnectionPort',
    'InterconnectionServiceToken',
    'OrganizationAddress',
    'ProjectBgpConfig',
    'GetDeviceBgpNeighborsBgpNeighborResult',
    'GetDeviceBgpNeighborsBgpNeighborRoutesInResult',
    'GetDeviceBgpNeighborsBgpNeighborRoutesOutResult',
    'GetDeviceNetworkResult',
    'GetDevicePortResult',
    'GetDevicesDeviceResult',
    'GetDevicesDeviceNetworkResult',
    'GetDevicesDevicePortResult',
    'GetDevicesFilterResult',
    'GetDevicesSortResult',
    'GetFacilityCapacityResult',
    'GetInterconnectionPortResult',
    'GetInterconnectionServiceTokenResult',
    'GetMetroCapacityResult',
    'GetOrganizationAddressResult',
    'GetPlansFilterResult',
    'GetPlansPlanResult',
    'GetPlansSortResult',
    'GetProjectBgpConfigResult',
]

@pulumi.output_type
class DeviceBehavior(dict):
    @staticmethod
    def __key_warning(key: str):
        suggest = None
        if key == "allowChanges":
            suggest = "allow_changes"

        if suggest:
            pulumi.log.warn(f"Key '{key}' not found in DeviceBehavior. Access the value via the '{suggest}' property getter instead.")

    def __getitem__(self, key: str) -> Any:
        DeviceBehavior.__key_warning(key)
        return super().__getitem__(key)

    def get(self, key: str, default = None) -> Any:
        DeviceBehavior.__key_warning(key)
        return super().get(key, default)

    def __init__(__self__, *,
                 allow_changes: Optional[Sequence[_builtins.str]] = None):
        """
        :param Sequence[_builtins.str] allow_changes: List of attributes that are allowed to change without recreating the instance. Supported attributes: `custom_data`, `user_data`
        """
        if allow_changes is not None:
            pulumi.set(__self__, "allow_changes", allow_changes)

    @_builtins.property
    @pulumi.getter(name="allowChanges")
    def allow_changes(self) -> Optional[Sequence[_builtins.str]]:
        """
        List of attributes that are allowed to change without recreating the instance. Supported attributes: `custom_data`, `user_data`
        """
        return pulumi.get(self, "allow_changes")


@pulumi.output_type
class DeviceIpAddress(dict):
    @staticmethod
    def __key_warning(key: str):
        suggest = None
        if key == "reservationIds":
            suggest = "reservation_ids"

        if suggest:
            pulumi.log.warn(f"Key '{key}' not found in DeviceIpAddress. Access the value via the '{suggest}' property getter instead.")

    def __getitem__(self, key: str) -> Any:
        DeviceIpAddress.__key_warning(key)
        return super().__getitem__(key)

    def get(self, key: str, default = None) -> Any:
        DeviceIpAddress.__key_warning(key)
        return super().get(key, default)

    def __init__(__self__, *,
                 type: _builtins.str,
                 cidr: Optional[_builtins.int] = None,
                 reservation_ids: Optional[Sequence[_builtins.str]] = None):
        """
        :param _builtins.str type: one of public*ipv4,private*ipv4,public_ipv6
        :param _builtins.int cidr: CIDR suffix for IP block assigned to this device
        :param Sequence[_builtins.str] reservation_ids: IDs of reservations to pick the blocks from
        """
        pulumi.set(__self__, "type", type)
        if cidr is not None:
            pulumi.set(__self__, "cidr", cidr)
        if reservation_ids is not None:
            pulumi.set(__self__, "reservation_ids", reservation_ids)

    @_builtins.property
    @pulumi.getter
    def type(self) -> _builtins.str:
        """
        one of public*ipv4,private*ipv4,public_ipv6
        """
        return pulumi.get(self, "type")

    @_builtins.property
    @pulumi.getter
    def cidr(self) -> Optional[_builtins.int]:
        """
        CIDR suffix for IP block assigned to this device
        """
        return pulumi.get(self, "cidr")

    @_builtins.property
    @pulumi.getter(name="reservationIds")
    def reservation_ids(self) -> Optional[Sequence[_builtins.str]]:
        """
        IDs of reservations to pick the blocks from
        """
        return pulumi.get(self, "reservation_ids")


@pulumi.output_type
class DeviceNetwork(dict):
    def __init__(__self__, *,
                 address: Optional[_builtins.str] = None,
                 cidr: Optional[_builtins.int] = None,
                 family: Optional[_builtins.int] = None,
                 gateway: Optional[_builtins.str] = None,
                 public: Optional[_builtins.bool] = None):
        """
        :param _builtins.str address: IPv4 or IPv6 address string
        :param _builtins.int cidr: CIDR suffix for IP address block to be assigned, i.e. amount of addresses
        :param _builtins.int family: IP version - "4" or "6"
        :param _builtins.str gateway: Address of router
        :param _builtins.bool public: Whether the address is routable from the Internet
        """
        if address is not None:
            pulumi.set(__self__, "address", address)
        if cidr is not None:
            pulumi.set(__self__, "cidr", cidr)
        if family is not None:
            pulumi.set(__self__, "family", family)
        if gateway is not None:
            pulumi.set(__self__, "gateway", gateway)
        if public is not None:
            pulumi.set(__self__, "public", public)

    @_builtins.property
    @pulumi.getter
    def address(self) -> Optional[_builtins.str]:
        """
        IPv4 or IPv6 address string
        """
        return pulumi.get(self, "address")

    @_builtins.property
    @pulumi.getter
    def cidr(self) -> Optional[_builtins.int]:
        """
        CIDR suffix for IP address block to be assigned, i.e. amount of addresses
        """
        return pulumi.get(self, "cidr")

    @_builtins.property
    @pulumi.getter
    def family(self) -> Optional[_builtins.int]:
        """
        IP version - "4" or "6"
        """
        return pulumi.get(self, "family")

    @_builtins.property
    @pulumi.getter
    def gateway(self) -> Optional[_builtins.str]:
        """
        Address of router
        """
        return pulumi.get(self, "gateway")

    @_builtins.property
    @pulumi.getter
    def public(self) -> Optional[_builtins.bool]:
        """
        Whether the address is routable from the Internet
        """
        return pulumi.get(self, "public")


@pulumi.output_type
class DevicePort(dict):
    def __init__(__self__, *,
                 bonded: Optional[_builtins.bool] = None,
                 id: Optional[_builtins.str] = None,
                 mac: Optional[_builtins.str] = None,
                 name: Optional[_builtins.str] = None,
                 type: Optional[_builtins.str] = None):
        """
        :param _builtins.bool bonded: Whether this port is part of a bond in bonded network setup
        :param _builtins.str id: The ID of the device
        :param _builtins.str mac: MAC address assigned to the port
        :param _builtins.str name: Name of the port (e.g. eth0, or bond0)
        :param _builtins.str type: One of [private_ipv4, public_ipv4, public_ipv6]
        """
        if bonded is not None:
            pulumi.set(__self__, "bonded", bonded)
        if id is not None:
            pulumi.set(__self__, "id", id)
        if mac is not None:
            pulumi.set(__self__, "mac", mac)
        if name is not None:
            pulumi.set(__self__, "name", name)
        if type is not None:
            pulumi.set(__self__, "type", type)

    @_builtins.property
    @pulumi.getter
    def bonded(self) -> Optional[_builtins.bool]:
        """
        Whether this port is part of a bond in bonded network setup
        """
        return pulumi.get(self, "bonded")

    @_builtins.property
    @pulumi.getter
    def id(self) -> Optional[_builtins.str]:
        """
        The ID of the device
        """
        return pulumi.get(self, "id")

    @_builtins.property
    @pulumi.getter
    def mac(self) -> Optional[_builtins.str]:
        """
        MAC address assigned to the port
        """
        return pulumi.get(self, "mac")

    @_builtins.property
    @pulumi.getter
    def name(self) -> Optional[_builtins.str]:
        """
        Name of the port (e.g. eth0, or bond0)
        """
        return pulumi.get(self, "name")

    @_builtins.property
    @pulumi.getter
    def type(self) -> Optional[_builtins.str]:
        """
        One of [private_ipv4, public_ipv4, public_ipv6]
        """
        return pulumi.get(self, "type")


@pulumi.output_type
class DeviceReinstall(dict):
    @staticmethod
    def __key_warning(key: str):
        suggest = None
        if key == "deprovisionFast":
            suggest = "deprovision_fast"
        elif key == "preserveData":
            suggest = "preserve_data"

        if suggest:
            pulumi.log.warn(f"Key '{key}' not found in DeviceReinstall. Access the value via the '{suggest}' property getter instead.")

    def __getitem__(self, key: str) -> Any:
        DeviceReinstall.__key_warning(key)
        return super().__getitem__(key)

    def get(self, key: str, default = None) -> Any:
        DeviceReinstall.__key_warning(key)
        return super().get(key, default)

    def __init__(__self__, *,
                 deprovision_fast: Optional[_builtins.bool] = None,
                 enabled: Optional[_builtins.bool] = None,
                 preserve_data: Optional[_builtins.bool] = None):
        """
        :param _builtins.bool deprovision_fast: Whether the OS disk should be filled with `00h` bytes before reinstall
        :param _builtins.bool enabled: Whether the device should be reinstalled instead of destroyed
        :param _builtins.bool preserve_data: Whether the non-OS disks should be kept or wiped during reinstall
        """
        if deprovision_fast is not None:
            pulumi.set(__self__, "deprovision_fast", deprovision_fast)
        if enabled is not None:
            pulumi.set(__self__, "enabled", enabled)
        if preserve_data is not None:
            pulumi.set(__self__, "preserve_data", preserve_data)

    @_builtins.property
    @pulumi.getter(name="deprovisionFast")
    def deprovision_fast(self) -> Optional[_builtins.bool]:
        """
        Whether the OS disk should be filled with `00h` bytes before reinstall
        """
        return pulumi.get(self, "deprovision_fast")

    @_builtins.property
    @pulumi.getter
    def enabled(self) -> Optional[_builtins.bool]:
        """
        Whether the device should be reinstalled instead of destroyed
        """
        return pulumi.get(self, "enabled")

    @_builtins.property
    @pulumi.getter(name="preserveData")
    def preserve_data(self) -> Optional[_builtins.bool]:
        """
        Whether the non-OS disks should be kept or wiped during reinstall
        """
        return pulumi.get(self, "preserve_data")


@pulumi.output_type
class GatewayTimeouts(dict):
    def __init__(__self__, *,
                 delete: Optional[_builtins.str] = None):
        """
        :param _builtins.str delete: A string that can be [parsed as a duration](https://pkg.go.dev/time#ParseDuration) consisting of numbers and unit suffixes, such as "30s" or "2h45m". Valid time units are "s" (seconds), "m" (minutes), "h" (hours). Setting a timeout for a Delete operation is only applicable if changes are saved into state before the destroy operation occurs.
        """
        if delete is not None:
            pulumi.set(__self__, "delete", delete)

    @_builtins.property
    @pulumi.getter
    def delete(self) -> Optional[_builtins.str]:
        """
        A string that can be [parsed as a duration](https://pkg.go.dev/time#ParseDuration) consisting of numbers and unit suffixes, such as "30s" or "2h45m". Valid time units are "s" (seconds), "m" (minutes), "h" (hours). Setting a timeout for a Delete operation is only applicable if changes are saved into state before the destroy operation occurs.
        """
        return pulumi.get(self, "delete")


@pulumi.output_type
class InterconnectionPort(dict):
    @staticmethod
    def __key_warning(key: str):
        suggest = None
        if key == "linkStatus":
            suggest = "link_status"
        elif key == "virtualCircuitIds":
            suggest = "virtual_circuit_ids"

        if suggest:
            pulumi.log.warn(f"Key '{key}' not found in InterconnectionPort. Access the value via the '{suggest}' property getter instead.")

    def __getitem__(self, key: str) -> Any:
        InterconnectionPort.__key_warning(key)
        return super().__getitem__(key)

    def get(self, key: str, default = None) -> Any:
        InterconnectionPort.__key_warning(key)
        return super().get(key, default)

    def __init__(__self__, *,
                 id: _builtins.str,
                 link_status: _builtins.str,
                 name: _builtins.str,
                 role: _builtins.str,
                 speed: _builtins.int,
                 status: _builtins.str,
                 virtual_circuit_ids: Sequence[_builtins.str]):
        pulumi.set(__self__, "id", id)
        pulumi.set(__self__, "link_status", link_status)
        pulumi.set(__self__, "name", name)
        pulumi.set(__self__, "role", role)
        pulumi.set(__self__, "speed", speed)
        pulumi.set(__self__, "status", status)
        pulumi.set(__self__, "virtual_circuit_ids", virtual_circuit_ids)

    @_builtins.property
    @pulumi.getter
    def id(self) -> _builtins.str:
        return pulumi.get(self, "id")

    @_builtins.property
    @pulumi.getter(name="linkStatus")
    def link_status(self) -> _builtins.str:
        return pulumi.get(self, "link_status")

    @_builtins.property
    @pulumi.getter
    def name(self) -> _builtins.str:
        return pulumi.get(self, "name")

    @_builtins.property
    @pulumi.getter
    def role(self) -> _builtins.str:
        return pulumi.get(self, "role")

    @_builtins.property
    @pulumi.getter
    def speed(self) -> _builtins.int:
        return pulumi.get(self, "speed")

    @_builtins.property
    @pulumi.getter
    def status(self) -> _builtins.str:
        return pulumi.get(self, "status")

    @_builtins.property
    @pulumi.getter(name="virtualCircuitIds")
    def virtual_circuit_ids(self) -> Sequence[_builtins.str]:
        return pulumi.get(self, "virtual_circuit_ids")


@pulumi.output_type
class InterconnectionServiceToken(dict):
    @staticmethod
    def __key_warning(key: str):
        suggest = None
        if key == "expiresAt":
            suggest = "expires_at"
        elif key == "maxAllowedSpeed":
            suggest = "max_allowed_speed"

        if suggest:
            pulumi.log.warn(f"Key '{key}' not found in InterconnectionServiceToken. Access the value via the '{suggest}' property getter instead.")

    def __getitem__(self, key: str) -> Any:
        InterconnectionServiceToken.__key_warning(key)
        return super().__getitem__(key)

    def get(self, key: str, default = None) -> Any:
        InterconnectionServiceToken.__key_warning(key)
        return super().get(key, default)

    def __init__(__self__, *,
                 expires_at: _builtins.str,
                 id: _builtins.str,
                 max_allowed_speed: _builtins.str,
                 role: _builtins.str,
                 state: _builtins.str,
                 type: _builtins.str):
        pulumi.set(__self__, "expires_at", expires_at)
        pulumi.set(__self__, "id", id)
        pulumi.set(__self__, "max_allowed_speed", max_allowed_speed)
        pulumi.set(__self__, "role", role)
        pulumi.set(__self__, "state", state)
        pulumi.set(__self__, "type", type)

    @_builtins.property
    @pulumi.getter(name="expiresAt")
    def expires_at(self) -> _builtins.str:
        return pulumi.get(self, "expires_at")

    @_builtins.property
    @pulumi.getter
    def id(self) -> _builtins.str:
        return pulumi.get(self, "id")

    @_builtins.property
    @pulumi.getter(name="maxAllowedSpeed")
    def max_allowed_speed(self) -> _builtins.str:
        return pulumi.get(self, "max_allowed_speed")

    @_builtins.property
    @pulumi.getter
    def role(self) -> _builtins.str:
        return pulumi.get(self, "role")

    @_builtins.property
    @pulumi.getter
    def state(self) -> _builtins.str:
        return pulumi.get(self, "state")

    @_builtins.property
    @pulumi.getter
    def type(self) -> _builtins.str:
        return pulumi.get(self, "type")


@pulumi.output_type
class OrganizationAddress(dict):
    @staticmethod
    def __key_warning(key: str):
        suggest = None
        if key == "zipCode":
            suggest = "zip_code"

        if suggest:
            pulumi.log.warn(f"Key '{key}' not found in OrganizationAddress. Access the value via the '{suggest}' property getter instead.")

    def __getitem__(self, key: str) -> Any:
        OrganizationAddress.__key_warning(key)
        return super().__getitem__(key)

    def get(self, key: str, default = None) -> Any:
        OrganizationAddress.__key_warning(key)
        return super().get(key, default)

    def __init__(__self__, *,
                 address: _builtins.str,
                 city: _builtins.str,
                 country: _builtins.str,
                 zip_code: _builtins.str,
                 state: Optional[_builtins.str] = None):
        """
        :param _builtins.str address: Postal address.
        :param _builtins.str city: City name.
        :param _builtins.str country: Two letter country code (ISO 3166-1 alpha-2), e.g. US.
        :param _builtins.str zip_code: Zip Code.
        :param _builtins.str state: State name.
        """
        pulumi.set(__self__, "address", address)
        pulumi.set(__self__, "city", city)
        pulumi.set(__self__, "country", country)
        pulumi.set(__self__, "zip_code", zip_code)
        if state is not None:
            pulumi.set(__self__, "state", state)

    @_builtins.property
    @pulumi.getter
    def address(self) -> _builtins.str:
        """
        Postal address.
        """
        return pulumi.get(self, "address")

    @_builtins.property
    @pulumi.getter
    def city(self) -> _builtins.str:
        """
        City name.
        """
        return pulumi.get(self, "city")

    @_builtins.property
    @pulumi.getter
    def country(self) -> _builtins.str:
        """
        Two letter country code (ISO 3166-1 alpha-2), e.g. US.
        """
        return pulumi.get(self, "country")

    @_builtins.property
    @pulumi.getter(name="zipCode")
    def zip_code(self) -> _builtins.str:
        """
        Zip Code.
        """
        return pulumi.get(self, "zip_code")

    @_builtins.property
    @pulumi.getter
    def state(self) -> Optional[_builtins.str]:
        """
        State name.
        """
        return pulumi.get(self, "state")


@pulumi.output_type
class ProjectBgpConfig(dict):
    @staticmethod
    def __key_warning(key: str):
        suggest = None
        if key == "deploymentType":
            suggest = "deployment_type"
        elif key == "maxPrefix":
            suggest = "max_prefix"

        if suggest:
            pulumi.log.warn(f"Key '{key}' not found in ProjectBgpConfig. Access the value via the '{suggest}' property getter instead.")

    def __getitem__(self, key: str) -> Any:
        ProjectBgpConfig.__key_warning(key)
        return super().__getitem__(key)

    def get(self, key: str, default = None) -> Any:
        ProjectBgpConfig.__key_warning(key)
        return super().get(key, default)

    def __init__(__self__, *,
                 asn: _builtins.int,
                 deployment_type: _builtins.str,
                 max_prefix: Optional[_builtins.int] = None,
                 md5: Optional[_builtins.str] = None,
                 status: Optional[_builtins.str] = None):
        """
        :param _builtins.int asn: Autonomous System Number for local BGP deployment.
        :param _builtins.str deployment_type: `local` or `global`, the `local` is likely to be usable immediately, the `global` will need to be reviewed by Equinix Metal engineers.
        :param _builtins.int max_prefix: The maximum number of route filters allowed per server.
        :param _builtins.str md5: Password for BGP session in plaintext (not a checksum).
        :param _builtins.str status: status of BGP configuration in the project.
        """
        pulumi.set(__self__, "asn", asn)
        pulumi.set(__self__, "deployment_type", deployment_type)
        if max_prefix is not None:
            pulumi.set(__self__, "max_prefix", max_prefix)
        if md5 is not None:
            pulumi.set(__self__, "md5", md5)
        if status is not None:
            pulumi.set(__self__, "status", status)

    @_builtins.property
    @pulumi.getter
    def asn(self) -> _builtins.int:
        """
        Autonomous System Number for local BGP deployment.
        """
        return pulumi.get(self, "asn")

    @_builtins.property
    @pulumi.getter(name="deploymentType")
    def deployment_type(self) -> _builtins.str:
        """
        `local` or `global`, the `local` is likely to be usable immediately, the `global` will need to be reviewed by Equinix Metal engineers.
        """
        return pulumi.get(self, "deployment_type")

    @_builtins.property
    @pulumi.getter(name="maxPrefix")
    def max_prefix(self) -> Optional[_builtins.int]:
        """
        The maximum number of route filters allowed per server.
        """
        return pulumi.get(self, "max_prefix")

    @_builtins.property
    @pulumi.getter
    def md5(self) -> Optional[_builtins.str]:
        """
        Password for BGP session in plaintext (not a checksum).
        """
        return pulumi.get(self, "md5")

    @_builtins.property
    @pulumi.getter
    def status(self) -> Optional[_builtins.str]:
        """
        status of BGP configuration in the project.
        """
        return pulumi.get(self, "status")


@pulumi.output_type
class GetDeviceBgpNeighborsBgpNeighborResult(dict):
    def __init__(__self__, *,
                 address_family: _builtins.int,
                 customer_as: _builtins.int,
                 customer_ip: _builtins.str,
                 md5_enabled: _builtins.bool,
                 md5_password: _builtins.str,
                 multihop: _builtins.bool,
                 peer_as: _builtins.int,
                 routes_ins: Sequence['outputs.GetDeviceBgpNeighborsBgpNeighborRoutesInResult'],
                 routes_outs: Sequence['outputs.GetDeviceBgpNeighborsBgpNeighborRoutesOutResult'],
                 peer_ips: Optional[Sequence[_builtins.str]] = None):
        """
        :param _builtins.int address_family: IP address version, 4 or 6.
        :param _builtins.int customer_as: Local autonomous system number.
        :param _builtins.str customer_ip: Local used peer IP address.
        :param _builtins.bool md5_enabled: Whether BGP session is password enabled.
        :param _builtins.str md5_password: BGP session password in plaintext (not a checksum).
        :param _builtins.bool multihop: Whether the neighbor is in EBGP multihop session.
        :param _builtins.int peer_as: Peer AS number (different than customer_as for EBGP).
        :param Sequence['GetDeviceBgpNeighborsBgpNeighborRoutesInArgs'] routes_ins: Array of incoming routes.
        :param Sequence['GetDeviceBgpNeighborsBgpNeighborRoutesOutArgs'] routes_outs: Array of outgoing routes in the same format.
        :param Sequence[_builtins.str] peer_ips: Array of IP addresses of this neighbor's peers.
        """
        pulumi.set(__self__, "address_family", address_family)
        pulumi.set(__self__, "customer_as", customer_as)
        pulumi.set(__self__, "customer_ip", customer_ip)
        pulumi.set(__self__, "md5_enabled", md5_enabled)
        pulumi.set(__self__, "md5_password", md5_password)
        pulumi.set(__self__, "multihop", multihop)
        pulumi.set(__self__, "peer_as", peer_as)
        pulumi.set(__self__, "routes_ins", routes_ins)
        pulumi.set(__self__, "routes_outs", routes_outs)
        if peer_ips is not None:
            pulumi.set(__self__, "peer_ips", peer_ips)

    @_builtins.property
    @pulumi.getter(name="addressFamily")
    def address_family(self) -> _builtins.int:
        """
        IP address version, 4 or 6.
        """
        return pulumi.get(self, "address_family")

    @_builtins.property
    @pulumi.getter(name="customerAs")
    def customer_as(self) -> _builtins.int:
        """
        Local autonomous system number.
        """
        return pulumi.get(self, "customer_as")

    @_builtins.property
    @pulumi.getter(name="customerIp")
    def customer_ip(self) -> _builtins.str:
        """
        Local used peer IP address.
        """
        return pulumi.get(self, "customer_ip")

    @_builtins.property
    @pulumi.getter(name="md5Enabled")
    def md5_enabled(self) -> _builtins.bool:
        """
        Whether BGP session is password enabled.
        """
        return pulumi.get(self, "md5_enabled")

    @_builtins.property
    @pulumi.getter(name="md5Password")
    def md5_password(self) -> _builtins.str:
        """
        BGP session password in plaintext (not a checksum).
        """
        return pulumi.get(self, "md5_password")

    @_builtins.property
    @pulumi.getter
    def multihop(self) -> _builtins.bool:
        """
        Whether the neighbor is in EBGP multihop session.
        """
        return pulumi.get(self, "multihop")

    @_builtins.property
    @pulumi.getter(name="peerAs")
    def peer_as(self) -> _builtins.int:
        """
        Peer AS number (different than customer_as for EBGP).
        """
        return pulumi.get(self, "peer_as")

    @_builtins.property
    @pulumi.getter(name="routesIns")
    def routes_ins(self) -> Sequence['outputs.GetDeviceBgpNeighborsBgpNeighborRoutesInResult']:
        """
        Array of incoming routes.
        """
        return pulumi.get(self, "routes_ins")

    @_builtins.property
    @pulumi.getter(name="routesOuts")
    def routes_outs(self) -> Sequence['outputs.GetDeviceBgpNeighborsBgpNeighborRoutesOutResult']:
        """
        Array of outgoing routes in the same format.
        """
        return pulumi.get(self, "routes_outs")

    @_builtins.property
    @pulumi.getter(name="peerIps")
    def peer_ips(self) -> Optional[Sequence[_builtins.str]]:
        """
        Array of IP addresses of this neighbor's peers.
        """
        return pulumi.get(self, "peer_ips")


@pulumi.output_type
class GetDeviceBgpNeighborsBgpNeighborRoutesInResult(dict):
    def __init__(__self__, *,
                 exact: _builtins.bool,
                 route: _builtins.str):
        """
        :param _builtins.bool exact: (bool) Whether the route is exact.
        :param _builtins.str route: CIDR expression of route (IP/mask).
        """
        pulumi.set(__self__, "exact", exact)
        pulumi.set(__self__, "route", route)

    @_builtins.property
    @pulumi.getter
    def exact(self) -> _builtins.bool:
        """
        (bool) Whether the route is exact.
        """
        return pulumi.get(self, "exact")

    @_builtins.property
    @pulumi.getter
    def route(self) -> _builtins.str:
        """
        CIDR expression of route (IP/mask).
        """
        return pulumi.get(self, "route")


@pulumi.output_type
class GetDeviceBgpNeighborsBgpNeighborRoutesOutResult(dict):
    def __init__(__self__, *,
                 exact: _builtins.bool,
                 route: _builtins.str):
        """
        :param _builtins.bool exact: (bool) Whether the route is exact.
        :param _builtins.str route: CIDR expression of route (IP/mask).
        """
        pulumi.set(__self__, "exact", exact)
        pulumi.set(__self__, "route", route)

    @_builtins.property
    @pulumi.getter
    def exact(self) -> _builtins.bool:
        """
        (bool) Whether the route is exact.
        """
        return pulumi.get(self, "exact")

    @_builtins.property
    @pulumi.getter
    def route(self) -> _builtins.str:
        """
        CIDR expression of route (IP/mask).
        """
        return pulumi.get(self, "route")


@pulumi.output_type
class GetDeviceNetworkResult(dict):
    def __init__(__self__, *,
                 address: _builtins.str,
                 cidr: _builtins.int,
                 family: _builtins.int,
                 gateway: _builtins.str,
                 public: _builtins.bool):
        """
        :param _builtins.str address: IPv4 or IPv6 address string
        :param _builtins.int cidr: Bit length of the network mask of the address
        :param _builtins.int family: IP version - "4" or "6"
        :param _builtins.str gateway: Address of router
        :param _builtins.bool public: Whether the address is routable from the Internet
        """
        pulumi.set(__self__, "address", address)
        pulumi.set(__self__, "cidr", cidr)
        pulumi.set(__self__, "family", family)
        pulumi.set(__self__, "gateway", gateway)
        pulumi.set(__self__, "public", public)

    @_builtins.property
    @pulumi.getter
    def address(self) -> _builtins.str:
        """
        IPv4 or IPv6 address string
        """
        return pulumi.get(self, "address")

    @_builtins.property
    @pulumi.getter
    def cidr(self) -> _builtins.int:
        """
        Bit length of the network mask of the address
        """
        return pulumi.get(self, "cidr")

    @_builtins.property
    @pulumi.getter
    def family(self) -> _builtins.int:
        """
        IP version - "4" or "6"
        """
        return pulumi.get(self, "family")

    @_builtins.property
    @pulumi.getter
    def gateway(self) -> _builtins.str:
        """
        Address of router
        """
        return pulumi.get(self, "gateway")

    @_builtins.property
    @pulumi.getter
    def public(self) -> _builtins.bool:
        """
        Whether the address is routable from the Internet
        """
        return pulumi.get(self, "public")


@pulumi.output_type
class GetDevicePortResult(dict):
    def __init__(__self__, *,
                 bonded: _builtins.bool,
                 id: _builtins.str,
                 mac: _builtins.str,
                 name: _builtins.str,
                 type: _builtins.str):
        """
        :param _builtins.bool bonded: Whether this port is part of a bond in bonded network setup
        :param _builtins.str id: The ID of the device
        :param _builtins.str mac: MAC address assigned to the port
        :param _builtins.str name: Name of the port (e.g. eth0, or bond0)
        :param _builtins.str type: Type of the port (e.g. NetworkPort or NetworkBondPort)
        """
        pulumi.set(__self__, "bonded", bonded)
        pulumi.set(__self__, "id", id)
        pulumi.set(__self__, "mac", mac)
        pulumi.set(__self__, "name", name)
        pulumi.set(__self__, "type", type)

    @_builtins.property
    @pulumi.getter
    def bonded(self) -> _builtins.bool:
        """
        Whether this port is part of a bond in bonded network setup
        """
        return pulumi.get(self, "bonded")

    @_builtins.property
    @pulumi.getter
    def id(self) -> _builtins.str:
        """
        The ID of the device
        """
        return pulumi.get(self, "id")

    @_builtins.property
    @pulumi.getter
    def mac(self) -> _builtins.str:
        """
        MAC address assigned to the port
        """
        return pulumi.get(self, "mac")

    @_builtins.property
    @pulumi.getter
    def name(self) -> _builtins.str:
        """
        Name of the port (e.g. eth0, or bond0)
        """
        return pulumi.get(self, "name")

    @_builtins.property
    @pulumi.getter
    def type(self) -> _builtins.str:
        """
        Type of the port (e.g. NetworkPort or NetworkBondPort)
        """
        return pulumi.get(self, "type")


@pulumi.output_type
class GetDevicesDeviceResult(dict):
    def __init__(__self__, *,
                 access_private_ipv4: _builtins.str,
                 access_public_ipv4: _builtins.str,
                 access_public_ipv6: _builtins.str,
                 always_pxe: _builtins.bool,
                 billing_cycle: _builtins.str,
                 description: _builtins.str,
                 device_id: _builtins.str,
                 facility: _builtins.str,
                 hardware_reservation_id: _builtins.str,
                 hostname: _builtins.str,
                 ipxe_script_url: _builtins.str,
                 metro: _builtins.str,
                 network_type: _builtins.str,
                 networks: Sequence['outputs.GetDevicesDeviceNetworkResult'],
                 operating_system: _builtins.str,
                 plan: _builtins.str,
                 ports: Sequence['outputs.GetDevicesDevicePortResult'],
                 project_id: _builtins.str,
                 root_password: _builtins.str,
                 sos_hostname: _builtins.str,
                 ssh_key_ids: Sequence[_builtins.str],
                 state: _builtins.str,
                 storage: _builtins.str,
                 tags: Sequence[_builtins.str]):
        """
        :param _builtins.str access_private_ipv4: The ipv4 private IP assigned to the device
        :param _builtins.str access_public_ipv4: The ipv4 management IP assigned to the device
        :param _builtins.str access_public_ipv6: The ipv6 management IP assigned to the device
        :param _builtins.str billing_cycle: The billing cycle of the device (monthly or hourly)
        :param _builtins.str description: Description string for the device
        :param _builtins.str device_id: Device ID
        :param _builtins.str facility: The facility where the device is deployed
        :param _builtins.str hardware_reservation_id: The id of hardware reservation which this device occupies
        :param _builtins.str hostname: The device name
        :param _builtins.str metro: The metro where the device is deployed
        :param _builtins.str network_type: L2 network type of the device, one oflayer3, hybrid, layer2-individual, layer2-bonded
        :param Sequence['GetDevicesDeviceNetworkArgs'] networks: The device's private and public IP (v4 and v6) network details. When a device is run without any special network configuration, it will have 3 networks: ublic IPv4 at equinix_metal_device.name.network.0, IPv6 at equinix_metal_device.name.network.1 and private IPv4 at equinix_metal_device.name.network.2. Elastic addresses then stack by type - an assigned public IPv4 will go after the management public IPv4 (to index 1), and will then shift the indices of the IPv6 and private IPv4. Assigned private IPv4 will go after the management private IPv4 (to the end of the network list).
        :param _builtins.str operating_system: The operating system running on the device
        :param _builtins.str plan: The hardware config of the device
        :param Sequence['GetDevicesDevicePortArgs'] ports: Ports assigned to the device
        :param _builtins.str project_id: ID of project containing the devices. Exactly one of `project_id` and `organization_id` must be set.
        :param _builtins.str root_password: Root password to the server (if still available)
        :param _builtins.str sos_hostname: The hostname to use for [Serial over SSH](https://deploy.equinix.com/developers/docs/metal/resilience-recovery/serial-over-ssh/) access to the device
        :param Sequence[_builtins.str] ssh_key_ids: List of IDs of SSH keys deployed in the device, can be both user or project SSH keys
        :param _builtins.str state: The state of the device
        :param Sequence[_builtins.str] tags: Tags attached to the device
        """
        pulumi.set(__self__, "access_private_ipv4", access_private_ipv4)
        pulumi.set(__self__, "access_public_ipv4", access_public_ipv4)
        pulumi.set(__self__, "access_public_ipv6", access_public_ipv6)
        pulumi.set(__self__, "always_pxe", always_pxe)
        pulumi.set(__self__, "billing_cycle", billing_cycle)
        pulumi.set(__self__, "description", description)
        pulumi.set(__self__, "device_id", device_id)
        pulumi.set(__self__, "facility", facility)
        pulumi.set(__self__, "hardware_reservation_id", hardware_reservation_id)
        pulumi.set(__self__, "hostname", hostname)
        pulumi.set(__self__, "ipxe_script_url", ipxe_script_url)
        pulumi.set(__self__, "metro", metro)
        pulumi.set(__self__, "network_type", network_type)
        pulumi.set(__self__, "networks", networks)
        pulumi.set(__self__, "operating_system", operating_system)
        pulumi.set(__self__, "plan", plan)
        pulumi.set(__self__, "ports", ports)
        pulumi.set(__self__, "project_id", project_id)
        pulumi.set(__self__, "root_password", root_password)
        pulumi.set(__self__, "sos_hostname", sos_hostname)
        pulumi.set(__self__, "ssh_key_ids", ssh_key_ids)
        pulumi.set(__self__, "state", state)
        pulumi.set(__self__, "storage", storage)
        pulumi.set(__self__, "tags", tags)

    @_builtins.property
    @pulumi.getter(name="accessPrivateIpv4")
    def access_private_ipv4(self) -> _builtins.str:
        """
        The ipv4 private IP assigned to the device
        """
        return pulumi.get(self, "access_private_ipv4")

    @_builtins.property
    @pulumi.getter(name="accessPublicIpv4")
    def access_public_ipv4(self) -> _builtins.str:
        """
        The ipv4 management IP assigned to the device
        """
        return pulumi.get(self, "access_public_ipv4")

    @_builtins.property
    @pulumi.getter(name="accessPublicIpv6")
    def access_public_ipv6(self) -> _builtins.str:
        """
        The ipv6 management IP assigned to the device
        """
        return pulumi.get(self, "access_public_ipv6")

    @_builtins.property
    @pulumi.getter(name="alwaysPxe")
    def always_pxe(self) -> _builtins.bool:
        return pulumi.get(self, "always_pxe")

    @_builtins.property
    @pulumi.getter(name="billingCycle")
    def billing_cycle(self) -> _builtins.str:
        """
        The billing cycle of the device (monthly or hourly)
        """
        return pulumi.get(self, "billing_cycle")

    @_builtins.property
    @pulumi.getter
    def description(self) -> _builtins.str:
        """
        Description string for the device
        """
        return pulumi.get(self, "description")

    @_builtins.property
    @pulumi.getter(name="deviceId")
    def device_id(self) -> _builtins.str:
        """
        Device ID
        """
        return pulumi.get(self, "device_id")

    @_builtins.property
    @pulumi.getter
    @_utilities.deprecated("""Use metro instead of facility.  For more information, read the migration guide: https://registry.terraform.io/providers/equinix/equinix/latest/docs/guides/migration_guide_facilities_to_metros_devices""")
    def facility(self) -> _builtins.str:
        """
        The facility where the device is deployed
        """
        return pulumi.get(self, "facility")

    @_builtins.property
    @pulumi.getter(name="hardwareReservationId")
    def hardware_reservation_id(self) -> _builtins.str:
        """
        The id of hardware reservation which this device occupies
        """
        return pulumi.get(self, "hardware_reservation_id")

    @_builtins.property
    @pulumi.getter
    def hostname(self) -> _builtins.str:
        """
        The device name
        """
        return pulumi.get(self, "hostname")

    @_builtins.property
    @pulumi.getter(name="ipxeScriptUrl")
    def ipxe_script_url(self) -> _builtins.str:
        return pulumi.get(self, "ipxe_script_url")

    @_builtins.property
    @pulumi.getter
    def metro(self) -> _builtins.str:
        """
        The metro where the device is deployed
        """
        return pulumi.get(self, "metro")

    @_builtins.property
    @pulumi.getter(name="networkType")
    def network_type(self) -> _builtins.str:
        """
        L2 network type of the device, one oflayer3, hybrid, layer2-individual, layer2-bonded
        """
        return pulumi.get(self, "network_type")

    @_builtins.property
    @pulumi.getter
    def networks(self) -> Sequence['outputs.GetDevicesDeviceNetworkResult']:
        """
        The device's private and public IP (v4 and v6) network details. When a device is run without any special network configuration, it will have 3 networks: ublic IPv4 at equinix_metal_device.name.network.0, IPv6 at equinix_metal_device.name.network.1 and private IPv4 at equinix_metal_device.name.network.2. Elastic addresses then stack by type - an assigned public IPv4 will go after the management public IPv4 (to index 1), and will then shift the indices of the IPv6 and private IPv4. Assigned private IPv4 will go after the management private IPv4 (to the end of the network list).
        """
        return pulumi.get(self, "networks")

    @_builtins.property
    @pulumi.getter(name="operatingSystem")
    def operating_system(self) -> _builtins.str:
        """
        The operating system running on the device
        """
        return pulumi.get(self, "operating_system")

    @_builtins.property
    @pulumi.getter
    def plan(self) -> _builtins.str:
        """
        The hardware config of the device
        """
        return pulumi.get(self, "plan")

    @_builtins.property
    @pulumi.getter
    def ports(self) -> Sequence['outputs.GetDevicesDevicePortResult']:
        """
        Ports assigned to the device
        """
        return pulumi.get(self, "ports")

    @_builtins.property
    @pulumi.getter(name="projectId")
    def project_id(self) -> _builtins.str:
        """
        ID of project containing the devices. Exactly one of `project_id` and `organization_id` must be set.
        """
        return pulumi.get(self, "project_id")

    @_builtins.property
    @pulumi.getter(name="rootPassword")
    def root_password(self) -> _builtins.str:
        """
        Root password to the server (if still available)
        """
        return pulumi.get(self, "root_password")

    @_builtins.property
    @pulumi.getter(name="sosHostname")
    def sos_hostname(self) -> _builtins.str:
        """
        The hostname to use for [Serial over SSH](https://deploy.equinix.com/developers/docs/metal/resilience-recovery/serial-over-ssh/) access to the device
        """
        return pulumi.get(self, "sos_hostname")

    @_builtins.property
    @pulumi.getter(name="sshKeyIds")
    def ssh_key_ids(self) -> Sequence[_builtins.str]:
        """
        List of IDs of SSH keys deployed in the device, can be both user or project SSH keys
        """
        return pulumi.get(self, "ssh_key_ids")

    @_builtins.property
    @pulumi.getter
    def state(self) -> _builtins.str:
        """
        The state of the device
        """
        return pulumi.get(self, "state")

    @_builtins.property
    @pulumi.getter
    def storage(self) -> _builtins.str:
        return pulumi.get(self, "storage")

    @_builtins.property
    @pulumi.getter
    def tags(self) -> Sequence[_builtins.str]:
        """
        Tags attached to the device
        """
        return pulumi.get(self, "tags")


@pulumi.output_type
class GetDevicesDeviceNetworkResult(dict):
    def __init__(__self__, *,
                 address: _builtins.str,
                 cidr: _builtins.int,
                 family: _builtins.int,
                 gateway: _builtins.str,
                 public: _builtins.bool):
        """
        :param _builtins.str address: IPv4 or IPv6 address string
        :param _builtins.int cidr: Bit length of the network mask of the address
        :param _builtins.int family: IP version - "4" or "6"
        :param _builtins.str gateway: Address of router
        :param _builtins.bool public: Whether the address is routable from the Internet
        """
        pulumi.set(__self__, "address", address)
        pulumi.set(__self__, "cidr", cidr)
        pulumi.set(__self__, "family", family)
        pulumi.set(__self__, "gateway", gateway)
        pulumi.set(__self__, "public", public)

    @_builtins.property
    @pulumi.getter
    def address(self) -> _builtins.str:
        """
        IPv4 or IPv6 address string
        """
        return pulumi.get(self, "address")

    @_builtins.property
    @pulumi.getter
    def cidr(self) -> _builtins.int:
        """
        Bit length of the network mask of the address
        """
        return pulumi.get(self, "cidr")

    @_builtins.property
    @pulumi.getter
    def family(self) -> _builtins.int:
        """
        IP version - "4" or "6"
        """
        return pulumi.get(self, "family")

    @_builtins.property
    @pulumi.getter
    def gateway(self) -> _builtins.str:
        """
        Address of router
        """
        return pulumi.get(self, "gateway")

    @_builtins.property
    @pulumi.getter
    def public(self) -> _builtins.bool:
        """
        Whether the address is routable from the Internet
        """
        return pulumi.get(self, "public")


@pulumi.output_type
class GetDevicesDevicePortResult(dict):
    def __init__(__self__, *,
                 bonded: _builtins.bool,
                 id: _builtins.str,
                 mac: _builtins.str,
                 name: _builtins.str,
                 type: _builtins.str):
        """
        :param _builtins.bool bonded: Whether this port is part of a bond in bonded network setup
        :param _builtins.str id: The ID of the device
        :param _builtins.str mac: MAC address assigned to the port
        :param _builtins.str name: Name of the port (e.g. eth0, or bond0)
        :param _builtins.str type: Type of the port (e.g. NetworkPort or NetworkBondPort)
        """
        pulumi.set(__self__, "bonded", bonded)
        pulumi.set(__self__, "id", id)
        pulumi.set(__self__, "mac", mac)
        pulumi.set(__self__, "name", name)
        pulumi.set(__self__, "type", type)

    @_builtins.property
    @pulumi.getter
    def bonded(self) -> _builtins.bool:
        """
        Whether this port is part of a bond in bonded network setup
        """
        return pulumi.get(self, "bonded")

    @_builtins.property
    @pulumi.getter
    def id(self) -> _builtins.str:
        """
        The ID of the device
        """
        return pulumi.get(self, "id")

    @_builtins.property
    @pulumi.getter
    def mac(self) -> _builtins.str:
        """
        MAC address assigned to the port
        """
        return pulumi.get(self, "mac")

    @_builtins.property
    @pulumi.getter
    def name(self) -> _builtins.str:
        """
        Name of the port (e.g. eth0, or bond0)
        """
        return pulumi.get(self, "name")

    @_builtins.property
    @pulumi.getter
    def type(self) -> _builtins.str:
        """
        Type of the port (e.g. NetworkPort or NetworkBondPort)
        """
        return pulumi.get(self, "type")


@pulumi.output_type
class GetDevicesFilterResult(dict):
    def __init__(__self__, *,
                 attribute: _builtins.str,
                 values: Sequence[_builtins.str],
                 all: Optional[_builtins.bool] = None,
                 match_by: Optional[_builtins.str] = None):
        """
        :param _builtins.str attribute: The attribute used to filter. Filter attributes are case-sensitive
        :param Sequence[_builtins.str] values: The filter values. Filter values are case-sensitive. If you specify multiple values for a filter, the values are joined with an OR by default, and the request returns all results that match any of the specified values
        :param _builtins.bool all: If is set to true, the values are joined with an AND, and the requests returns only the results that match all specified values. Default is `false`.
               
               All fields in the `devices` block defined below can be used as attribute for both `sort` and `filter` blocks.
        :param _builtins.str match_by: The type of comparison to apply. One of: `in` , `re`, `substring`, `less_than`, `less_than_or_equal`, `greater_than`, `greater_than_or_equal`. Default is `in`.
        """
        pulumi.set(__self__, "attribute", attribute)
        pulumi.set(__self__, "values", values)
        if all is not None:
            pulumi.set(__self__, "all", all)
        if match_by is not None:
            pulumi.set(__self__, "match_by", match_by)

    @_builtins.property
    @pulumi.getter
    def attribute(self) -> _builtins.str:
        """
        The attribute used to filter. Filter attributes are case-sensitive
        """
        return pulumi.get(self, "attribute")

    @_builtins.property
    @pulumi.getter
    def values(self) -> Sequence[_builtins.str]:
        """
        The filter values. Filter values are case-sensitive. If you specify multiple values for a filter, the values are joined with an OR by default, and the request returns all results that match any of the specified values
        """
        return pulumi.get(self, "values")

    @_builtins.property
    @pulumi.getter
    def all(self) -> Optional[_builtins.bool]:
        """
        If is set to true, the values are joined with an AND, and the requests returns only the results that match all specified values. Default is `false`.

        All fields in the `devices` block defined below can be used as attribute for both `sort` and `filter` blocks.
        """
        return pulumi.get(self, "all")

    @_builtins.property
    @pulumi.getter(name="matchBy")
    def match_by(self) -> Optional[_builtins.str]:
        """
        The type of comparison to apply. One of: `in` , `re`, `substring`, `less_than`, `less_than_or_equal`, `greater_than`, `greater_than_or_equal`. Default is `in`.
        """
        return pulumi.get(self, "match_by")


@pulumi.output_type
class GetDevicesSortResult(dict):
    def __init__(__self__, *,
                 attribute: _builtins.str,
                 direction: Optional[_builtins.str] = None):
        """
        :param _builtins.str attribute: The attribute used to sort the results. Sort attributes are case-sensitive
        :param _builtins.str direction: Sort results in ascending or descending order. Strings are sorted in alphabetical order. One of: asc, desc
        """
        pulumi.set(__self__, "attribute", attribute)
        if direction is not None:
            pulumi.set(__self__, "direction", direction)

    @_builtins.property
    @pulumi.getter
    def attribute(self) -> _builtins.str:
        """
        The attribute used to sort the results. Sort attributes are case-sensitive
        """
        return pulumi.get(self, "attribute")

    @_builtins.property
    @pulumi.getter
    def direction(self) -> Optional[_builtins.str]:
        """
        Sort results in ascending or descending order. Strings are sorted in alphabetical order. One of: asc, desc
        """
        return pulumi.get(self, "direction")


@pulumi.output_type
class GetFacilityCapacityResult(dict):
    def __init__(__self__, *,
                 plan: _builtins.str,
                 quantity: Optional[_builtins.int] = None):
        """
        :param _builtins.str plan: Device plan that must be available in selected location.
        :param _builtins.int quantity: Minimun number of devices that must be available in selected location. Default is `1`.
        """
        pulumi.set(__self__, "plan", plan)
        if quantity is not None:
            pulumi.set(__self__, "quantity", quantity)

    @_builtins.property
    @pulumi.getter
    def plan(self) -> _builtins.str:
        """
        Device plan that must be available in selected location.
        """
        return pulumi.get(self, "plan")

    @_builtins.property
    @pulumi.getter
    def quantity(self) -> Optional[_builtins.int]:
        """
        Minimun number of devices that must be available in selected location. Default is `1`.
        """
        return pulumi.get(self, "quantity")


@pulumi.output_type
class GetInterconnectionPortResult(dict):
    def __init__(__self__, *,
                 id: _builtins.str,
                 link_status: _builtins.str,
                 name: _builtins.str,
                 role: _builtins.str,
                 speed: _builtins.int,
                 status: _builtins.str,
                 virtual_circuit_ids: Sequence[_builtins.str]):
        """
        :param _builtins.str id: Port UUID.
        :param _builtins.str link_status: Port link status.
        :param _builtins.str name: Port name.
        :param _builtins.str role: Port role - primary or secondary.
        :param _builtins.int speed: Port speed in bits per second.
        :param _builtins.str status: Port status.
        :param Sequence[_builtins.str] virtual_circuit_ids: List of IDs of virtual cicruits attached to this port.
        """
        pulumi.set(__self__, "id", id)
        pulumi.set(__self__, "link_status", link_status)
        pulumi.set(__self__, "name", name)
        pulumi.set(__self__, "role", role)
        pulumi.set(__self__, "speed", speed)
        pulumi.set(__self__, "status", status)
        pulumi.set(__self__, "virtual_circuit_ids", virtual_circuit_ids)

    @_builtins.property
    @pulumi.getter
    def id(self) -> _builtins.str:
        """
        Port UUID.
        """
        return pulumi.get(self, "id")

    @_builtins.property
    @pulumi.getter(name="linkStatus")
    def link_status(self) -> _builtins.str:
        """
        Port link status.
        """
        return pulumi.get(self, "link_status")

    @_builtins.property
    @pulumi.getter
    def name(self) -> _builtins.str:
        """
        Port name.
        """
        return pulumi.get(self, "name")

    @_builtins.property
    @pulumi.getter
    def role(self) -> _builtins.str:
        """
        Port role - primary or secondary.
        """
        return pulumi.get(self, "role")

    @_builtins.property
    @pulumi.getter
    def speed(self) -> _builtins.int:
        """
        Port speed in bits per second.
        """
        return pulumi.get(self, "speed")

    @_builtins.property
    @pulumi.getter
    def status(self) -> _builtins.str:
        """
        Port status.
        """
        return pulumi.get(self, "status")

    @_builtins.property
    @pulumi.getter(name="virtualCircuitIds")
    def virtual_circuit_ids(self) -> Sequence[_builtins.str]:
        """
        List of IDs of virtual cicruits attached to this port.
        """
        return pulumi.get(self, "virtual_circuit_ids")


@pulumi.output_type
class GetInterconnectionServiceTokenResult(dict):
    def __init__(__self__, *,
                 expires_at: _builtins.str,
                 id: _builtins.str,
                 max_allowed_speed: _builtins.str,
                 role: _builtins.str,
                 state: _builtins.str,
                 type: _builtins.str):
        """
        :param _builtins.str expires_at: Expiration date of the service token.
        :param _builtins.str id: Port UUID.
        :param _builtins.str max_allowed_speed: Maximum allowed speed for the service token, string like in the `speed` attribute.
        :param _builtins.str role: Port role - primary or secondary.
        :param _builtins.str type: Token type, `a_side` or `z_side`.
        """
        pulumi.set(__self__, "expires_at", expires_at)
        pulumi.set(__self__, "id", id)
        pulumi.set(__self__, "max_allowed_speed", max_allowed_speed)
        pulumi.set(__self__, "role", role)
        pulumi.set(__self__, "state", state)
        pulumi.set(__self__, "type", type)

    @_builtins.property
    @pulumi.getter(name="expiresAt")
    def expires_at(self) -> _builtins.str:
        """
        Expiration date of the service token.
        """
        return pulumi.get(self, "expires_at")

    @_builtins.property
    @pulumi.getter
    def id(self) -> _builtins.str:
        """
        Port UUID.
        """
        return pulumi.get(self, "id")

    @_builtins.property
    @pulumi.getter(name="maxAllowedSpeed")
    def max_allowed_speed(self) -> _builtins.str:
        """
        Maximum allowed speed for the service token, string like in the `speed` attribute.
        """
        return pulumi.get(self, "max_allowed_speed")

    @_builtins.property
    @pulumi.getter
    def role(self) -> _builtins.str:
        """
        Port role - primary or secondary.
        """
        return pulumi.get(self, "role")

    @_builtins.property
    @pulumi.getter
    def state(self) -> _builtins.str:
        return pulumi.get(self, "state")

    @_builtins.property
    @pulumi.getter
    def type(self) -> _builtins.str:
        """
        Token type, `a_side` or `z_side`.
        """
        return pulumi.get(self, "type")


@pulumi.output_type
class GetMetroCapacityResult(dict):
    def __init__(__self__, *,
                 plan: _builtins.str,
                 quantity: Optional[_builtins.int] = None):
        """
        :param _builtins.str plan: Device plan that must be available in selected location.
        :param _builtins.int quantity: Minimum number of devices that must be available in selected location. Default is `1`.
        """
        pulumi.set(__self__, "plan", plan)
        if quantity is not None:
            pulumi.set(__self__, "quantity", quantity)

    @_builtins.property
    @pulumi.getter
    def plan(self) -> _builtins.str:
        """
        Device plan that must be available in selected location.
        """
        return pulumi.get(self, "plan")

    @_builtins.property
    @pulumi.getter
    def quantity(self) -> Optional[_builtins.int]:
        """
        Minimum number of devices that must be available in selected location. Default is `1`.
        """
        return pulumi.get(self, "quantity")


@pulumi.output_type
class GetOrganizationAddressResult(dict):
    def __init__(__self__, *,
                 address: _builtins.str,
                 city: _builtins.str,
                 country: _builtins.str,
                 state: _builtins.str,
                 zip_code: _builtins.str):
        """
        :param _builtins.str address: Postal address.
        :param _builtins.str city: City name.
        :param _builtins.str country: Two letter country code (ISO 3166-1 alpha-2), e.g. US.
        :param _builtins.str state: State name.
        :param _builtins.str zip_code: Zip Code.
        """
        pulumi.set(__self__, "address", address)
        pulumi.set(__self__, "city", city)
        pulumi.set(__self__, "country", country)
        pulumi.set(__self__, "state", state)
        pulumi.set(__self__, "zip_code", zip_code)

    @_builtins.property
    @pulumi.getter
    def address(self) -> _builtins.str:
        """
        Postal address.
        """
        return pulumi.get(self, "address")

    @_builtins.property
    @pulumi.getter
    def city(self) -> _builtins.str:
        """
        City name.
        """
        return pulumi.get(self, "city")

    @_builtins.property
    @pulumi.getter
    def country(self) -> _builtins.str:
        """
        Two letter country code (ISO 3166-1 alpha-2), e.g. US.
        """
        return pulumi.get(self, "country")

    @_builtins.property
    @pulumi.getter
    def state(self) -> _builtins.str:
        """
        State name.
        """
        return pulumi.get(self, "state")

    @_builtins.property
    @pulumi.getter(name="zipCode")
    def zip_code(self) -> _builtins.str:
        """
        Zip Code.
        """
        return pulumi.get(self, "zip_code")


@pulumi.output_type
class GetPlansFilterResult(dict):
    def __init__(__self__, *,
                 attribute: _builtins.str,
                 values: Sequence[_builtins.str],
                 all: Optional[_builtins.bool] = None,
                 match_by: Optional[_builtins.str] = None):
        """
        :param _builtins.str attribute: The attribute used to filter. Filter attributes are case-sensitive
        :param Sequence[_builtins.str] values: The filter values. Filter values are case-sensitive. If you specify multiple values for a filter, the values are joined with an OR by default, and the request returns all results that match any of the specified values
        :param _builtins.bool all: If is set to true, the values are joined with an AND, and the requests returns only the results that match all specified values. Default is `false`.
               
               All fields in the `plans` block defined below can be used as attribute for both `sort` and `filter` blocks.
        :param _builtins.str match_by: The type of comparison to apply. One of: `in` , `re`, `substring`, `less_than`, `less_than_or_equal`, `greater_than`, `greater_than_or_equal`. Default is `in`.
        """
        pulumi.set(__self__, "attribute", attribute)
        pulumi.set(__self__, "values", values)
        if all is not None:
            pulumi.set(__self__, "all", all)
        if match_by is not None:
            pulumi.set(__self__, "match_by", match_by)

    @_builtins.property
    @pulumi.getter
    def attribute(self) -> _builtins.str:
        """
        The attribute used to filter. Filter attributes are case-sensitive
        """
        return pulumi.get(self, "attribute")

    @_builtins.property
    @pulumi.getter
    def values(self) -> Sequence[_builtins.str]:
        """
        The filter values. Filter values are case-sensitive. If you specify multiple values for a filter, the values are joined with an OR by default, and the request returns all results that match any of the specified values
        """
        return pulumi.get(self, "values")

    @_builtins.property
    @pulumi.getter
    def all(self) -> Optional[_builtins.bool]:
        """
        If is set to true, the values are joined with an AND, and the requests returns only the results that match all specified values. Default is `false`.

        All fields in the `plans` block defined below can be used as attribute for both `sort` and `filter` blocks.
        """
        return pulumi.get(self, "all")

    @_builtins.property
    @pulumi.getter(name="matchBy")
    def match_by(self) -> Optional[_builtins.str]:
        """
        The type of comparison to apply. One of: `in` , `re`, `substring`, `less_than`, `less_than_or_equal`, `greater_than`, `greater_than_or_equal`. Default is `in`.
        """
        return pulumi.get(self, "match_by")


@pulumi.output_type
class GetPlansPlanResult(dict):
    def __init__(__self__, *,
                 available_in_metros: Sequence[_builtins.str],
                 available_ins: Sequence[_builtins.str],
                 class_: _builtins.str,
                 deployment_types: Sequence[_builtins.str],
                 description: _builtins.str,
                 id: _builtins.str,
                 legacy: _builtins.bool,
                 line: _builtins.str,
                 name: _builtins.str,
                 pricing_hour: _builtins.float,
                 pricing_month: _builtins.float,
                 slug: _builtins.str):
        """
        :param Sequence[_builtins.str] available_in_metros: list of metros where the plan is available
        :param Sequence[_builtins.str] available_ins: (**Deprecated**) list of facilities where the plan is available
        :param _builtins.str class_: plan class
        :param Sequence[_builtins.str] deployment_types: list of deployment types, e.g. on_demand, spot_market
        :param _builtins.str description: description of the plan
        :param _builtins.str id: id of the plan
        :param _builtins.bool legacy: flag showing if it's a legacy plan
        :param _builtins.str line: plan line, e.g. baremetal
        :param _builtins.str name: name of the plan
        :param _builtins.float pricing_hour: plan hourly price
        :param _builtins.float pricing_month: plan monthly price
        :param _builtins.str slug: plan slug
        """
        pulumi.set(__self__, "available_in_metros", available_in_metros)
        pulumi.set(__self__, "available_ins", available_ins)
        pulumi.set(__self__, "class_", class_)
        pulumi.set(__self__, "deployment_types", deployment_types)
        pulumi.set(__self__, "description", description)
        pulumi.set(__self__, "id", id)
        pulumi.set(__self__, "legacy", legacy)
        pulumi.set(__self__, "line", line)
        pulumi.set(__self__, "name", name)
        pulumi.set(__self__, "pricing_hour", pricing_hour)
        pulumi.set(__self__, "pricing_month", pricing_month)
        pulumi.set(__self__, "slug", slug)

    @_builtins.property
    @pulumi.getter(name="availableInMetros")
    def available_in_metros(self) -> Sequence[_builtins.str]:
        """
        list of metros where the plan is available
        """
        return pulumi.get(self, "available_in_metros")

    @_builtins.property
    @pulumi.getter(name="availableIns")
    @_utilities.deprecated("""Use available_in_metros instead.  For more information, read the migration guide: https://registry.terraform.io/providers/equinix/equinix/latest/docs/guides/migration_guide_facilities_to_metros_devices""")
    def available_ins(self) -> Sequence[_builtins.str]:
        """
        (**Deprecated**) list of facilities where the plan is available
        """
        return pulumi.get(self, "available_ins")

    @_builtins.property
    @pulumi.getter(name="class")
    def class_(self) -> _builtins.str:
        """
        plan class
        """
        return pulumi.get(self, "class_")

    @_builtins.property
    @pulumi.getter(name="deploymentTypes")
    def deployment_types(self) -> Sequence[_builtins.str]:
        """
        list of deployment types, e.g. on_demand, spot_market
        """
        return pulumi.get(self, "deployment_types")

    @_builtins.property
    @pulumi.getter
    def description(self) -> _builtins.str:
        """
        description of the plan
        """
        return pulumi.get(self, "description")

    @_builtins.property
    @pulumi.getter
    def id(self) -> _builtins.str:
        """
        id of the plan
        """
        return pulumi.get(self, "id")

    @_builtins.property
    @pulumi.getter
    def legacy(self) -> _builtins.bool:
        """
        flag showing if it's a legacy plan
        """
        return pulumi.get(self, "legacy")

    @_builtins.property
    @pulumi.getter
    def line(self) -> _builtins.str:
        """
        plan line, e.g. baremetal
        """
        return pulumi.get(self, "line")

    @_builtins.property
    @pulumi.getter
    def name(self) -> _builtins.str:
        """
        name of the plan
        """
        return pulumi.get(self, "name")

    @_builtins.property
    @pulumi.getter(name="pricingHour")
    def pricing_hour(self) -> _builtins.float:
        """
        plan hourly price
        """
        return pulumi.get(self, "pricing_hour")

    @_builtins.property
    @pulumi.getter(name="pricingMonth")
    def pricing_month(self) -> _builtins.float:
        """
        plan monthly price
        """
        return pulumi.get(self, "pricing_month")

    @_builtins.property
    @pulumi.getter
    def slug(self) -> _builtins.str:
        """
        plan slug
        """
        return pulumi.get(self, "slug")


@pulumi.output_type
class GetPlansSortResult(dict):
    def __init__(__self__, *,
                 attribute: _builtins.str,
                 direction: Optional[_builtins.str] = None):
        """
        :param _builtins.str attribute: The attribute used to sort the results. Sort attributes are case-sensitive
        :param _builtins.str direction: Sort results in ascending or descending order. Strings are sorted in alphabetical order. One of: asc, desc
        """
        pulumi.set(__self__, "attribute", attribute)
        if direction is not None:
            pulumi.set(__self__, "direction", direction)

    @_builtins.property
    @pulumi.getter
    def attribute(self) -> _builtins.str:
        """
        The attribute used to sort the results. Sort attributes are case-sensitive
        """
        return pulumi.get(self, "attribute")

    @_builtins.property
    @pulumi.getter
    def direction(self) -> Optional[_builtins.str]:
        """
        Sort results in ascending or descending order. Strings are sorted in alphabetical order. One of: asc, desc
        """
        return pulumi.get(self, "direction")


@pulumi.output_type
class GetProjectBgpConfigResult(dict):
    def __init__(__self__, *,
                 asn: _builtins.int,
                 deployment_type: _builtins.str,
                 max_prefix: _builtins.int,
                 md5: _builtins.str,
                 status: _builtins.str):
        """
        :param _builtins.int asn: Autonomous System Number for local BGP deployment.
        :param _builtins.str deployment_type: One of `private`, `public`.
        :param _builtins.int max_prefix: The maximum number of route filters allowed per server.
        :param _builtins.str md5: Password for BGP session in plaintext (not a checksum).
        :param _builtins.str status: Status of BGP configuration in the project.
        """
        pulumi.set(__self__, "asn", asn)
        pulumi.set(__self__, "deployment_type", deployment_type)
        pulumi.set(__self__, "max_prefix", max_prefix)
        pulumi.set(__self__, "md5", md5)
        pulumi.set(__self__, "status", status)

    @_builtins.property
    @pulumi.getter
    def asn(self) -> _builtins.int:
        """
        Autonomous System Number for local BGP deployment.
        """
        return pulumi.get(self, "asn")

    @_builtins.property
    @pulumi.getter(name="deploymentType")
    def deployment_type(self) -> _builtins.str:
        """
        One of `private`, `public`.
        """
        return pulumi.get(self, "deployment_type")

    @_builtins.property
    @pulumi.getter(name="maxPrefix")
    def max_prefix(self) -> _builtins.int:
        """
        The maximum number of route filters allowed per server.
        """
        return pulumi.get(self, "max_prefix")

    @_builtins.property
    @pulumi.getter
    def md5(self) -> _builtins.str:
        """
        Password for BGP session in plaintext (not a checksum).
        """
        return pulumi.get(self, "md5")

    @_builtins.property
    @pulumi.getter
    def status(self) -> _builtins.str:
        """
        Status of BGP configuration in the project.
        """
        return pulumi.get(self, "status")


