...
generate_examples_and_docs() {
    ...
    for yaml_file in "${EXISTING_PULUMI_EXAMPLES[@]}"; do
        ...
        echo "Generating examples and docs for: $NAME" >&2
...
merge_example_files() {
    ...
    echo "Generating merged example: $RESULT_FILE_NAME" >&2
    ...
    echo "Merged file generated: $MERGED_FILE" >&2
}
...
if [ ${#EXCLUDED_TF_EXAMPLES[@]} -gt 0 ]; then
    echo -e "\n\033[0;33m Warning:\033[0m The following examples were not converted due to their complexity:" >&2
    for excluded_file in "${EXCLUDED_TF_EXAMPLES[@]}"; do
        echo "  - $excluded_file" >&2
    done
    echo "Please manually update the Pulumi.yaml file for these resources in their respective directories in the examples section if any updates are expected." >&2
fi
